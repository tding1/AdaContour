    ...
    if len(contours) > 1:
        ...
        for key in groups.keys():
            ...
            if max_index != len(groups[key]) - 1:
                groups[key] = (
                    groups[key][max_index + 1 :] + groups[key][: max_index + 1]
                )
        ...
        degStartList = []
        for i in range(len(start_list)):
            deg = -np.arctan2(1, 0) + np.arctan2(
                start_list[i][0] - center_x, start_list[i][1] - center_y
            )
            deg = deg * 180 / np.pi
            if deg < 0:
                deg += 360
...
def getOrientedPoints(instance, mode):
    # first get center point
    instance = instance.astype(np.uint8)
    if mode == "ese_ori":
        # your implementation, return a tuple or a list center = (center_x, center_y)
        center_x, center_y = centerdot(instance)
    elif mode == "ese_box_center":
        center_x, center_y = (instance.shape[0] - 1) / 2, (instance.shape[1] - 1) / 2
    elif mode == "centroid":
        center_x, center_y = get_centroid(instance)
    elif mode == "inscribed_circle":
        center_x, center_y = get_inscribed_center(instance)
    elif mode == "hybrid":
...
def runOneImage(data, seg_mask, mode):
    objects_info = {}

    instance_mask = to_np(seg_mask).squeeze()
    img_height, img_width = instance_mask.shape
    instance_second = np.copy(instance_mask)

    # TD: make sure object is enclosed
    instance_second[0, :] = instance_second[-1, :] = instance_second[
        :, 0
    ] = instance_second[:, -1] = 0

    bbox = to_np(data[0]).reshape(-1)
    ...
    try:
        points, center_x, center_y = getOrientedPoints(instance_second, mode)
        objects_info["bbox"] = (x, y, w, h)
        objects_info["img_wh"] = (img_width, img_height)
        objects_info["center"] = (center_x, center_y)
        objects_info["r"] = points
        objects_info["check"] = False
    except:
        objects_info["check"] = True
