...
def get_gradient2(im):
    dist_map = cv2.distanceTransform(im, cv2.DIST_L2, cv2.DIST_MASK_PRECISE)
...
def get_startpoint(gradient_map, direction=0, bias=0):
    ...
    h_location, w_location = np.nonzero(
        gradient_map)[0], np.nonzero(gradient_map)[1]
...
def serach_box(search_map, index_h, index_w, scale):
    present_box = search_map[index_h-scale:index_h +
                             scale+1, index_w - scale:index_w+scale+1]
...
def get_boundingorder(gradient_map, direction=0):  # im_name is used to visualize
    ...
    orderlist = []
    (start_pointh, start_pointw) = get_startpoint(
        search_map, direction=direction)
    ...
    while present_point == start_point:
        if search_map[start_pointh][start_pointw+1] == 1:
            ...
            bias += 1
            (start_pointh, start_pointw) = get_startpoint(
                search_map, direction=direction, bias=bias)
            start_point = (start_pointh, start_pointw)
    ...
    while present_point != start_point:
        ...
        next_index_h, next_index_w = next_index[0][0] - \
            1+index_h, next_index[1][0]-1+index_w
    ...
    for w_index in range(polygon_map.shape[1]):
        ...
        for h_index in range(polygon_map.shape[0]):
            if (polygon_map[h_index][w_index] != 0) and (len(point_pairs) == 0):
                ...
                point_pairs.append((h_index, w_index))
                polygon_map = link_line(
                    point_pairs[0], point_pairs[1], polygon_map)
                point_pairs = []
                point_pairs.append((h_index, w_index))
            else:
                continue
