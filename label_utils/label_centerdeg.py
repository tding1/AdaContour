    ...
    if len(contours) > 1:
        ...
        for i in range(w+1):
            if distanceMapUp[i] < np.inf:
                ...
                selected_info[str(e_x)+"_"+str(e_y)
                              ] = dictEdgePoint[str(e_x)+"_"+str(e_y)]
        for i in range(h+1):
        ...
        for key in groups.keys():
            ...
            distGroup = np.diag(distance.cdist(
                groups[key], tempGroup, 'euclidean'))
...
def getOrientedPoints(instance):
    ...
    edges = get_gradient(instance)
    index_h, index_w = np.where(edges == 1)
    edgepoints_array = np.array([(index_w[i], index_h[i])
                                for i in range(len(index_h))])  # x, y
    ...
    for i in range(len(index_h)):
        ...
        deg_1 = -np.arctan2(1, 0) + \
            np.arctan2(index_w[i]-center_x, index_h[i]-center_y)
        deg_1 = deg_1 * 180 / np.pi
        if deg_1 < 0:
            deg_1 += 360
        deg_2 = -np.arctan2(1, 0) + \
            np.arctan2(index_w[i]+1-center_x, index_h[i]+1-center_y)
        deg_2 = deg_2 * 180 / np.pi
        if deg_2 < 0:
            deg_2 += 360
        deg_3 = -np.arctan2(1, 0) + \
            np.arctan2(index_w[i]-center_x, index_h[i]+1-center_y)
        deg_3 = deg_3 * 180 / np.pi
        if deg_3 < 0:
            deg_3 += 360
        deg_4 = -np.arctan2(1, 0) + \
            np.arctan2(index_w[i]+1-center_x, index_h[i]-center_y)
