...
class Dataset_coco(Dataset):
    ...
    def get_image(self, idx):
        img = cv2.imread(self.cfg.img_dir +
                         self.datalist[idx] + '.jpg')[:, :, [2, 1, 0]]
    ...
    def get_label(self, idx):
        ...
        for i in range(len(self.seg_label)):
            ...
            cen = np.array([(bb_pts[0] + bb_pts[2])/2,
                           (bb_pts[1] + bb_pts[3])/2])
            cen_label.append(cen.reshape(-1, 2))

        for j in range(len(self.bb_label)):
            bb_pts = np.array(self.bb_label[j][:-1])
            bbox = np.array([bb_pts[0], bb_pts[1], bb_pts[2] -
                            bb_pts[0], bb_pts[3]-bb_pts[1]])
    ...
    def get_cropped_data(self, img, label, cen, bbox, id):
        ...
        for i in range(len(label)):
            out = {}
            crop_im, h, w = self.sample_target_SE(
                img, bbox[i], search_area_factor=1.0, out_h=self.cfg.crop_h, out_w=self.cfg.crop_w)
            crop_im = torch.from_numpy(crop_im).permute(
                2, 0, 1).type(torch.float)[[2, 1, 0], :, :] / 255
            crop_im_norm = self.normalize(crop_im)

            mask = self.make_polygon_mask(label[i])
            mask, h, w = self.sample_target_SE(to_3D_np(
                mask), bbox[i], search_area_factor=1.0, out_h=self.cfg.crop_h, out_w=self.cfg.crop_w)

            out['cropped_img_rgb'] = crop_im
            out['cropped_img'] = crop_im_norm
            out['seg_mask'] = mask[:, :, 0]
            out['crop_h'], out['crop_w'] = self.cfg.crop_h, self.cfg.crop_w
            out['bbox_center'] = np.array(
                [(self.cfg.crop_w - 1) / 2, (self.cfg.crop_h - 1) / 2]).astype(np.float32)
            out['bbox'] = np.array(
                [0, 0, self.cfg.crop_w - 1, self.cfg.crop_h - 1]).astype(np.float32)
            out['id'] = id[i]
            outs.append(out)
    ...
    def sample_target_SE(self, im, target_bb, search_area_factor=2.0, out_h=256, out_w=256, mode=0):
        ...
        im_crop_padded = cv2.copyMakeBorder(
            im_crop, y1_pad, y2_pad, x1_pad, x2_pad, mode)
