...
class Visualize(object):
    ...
    def update_image(self, img, name="img", normalized=True):
        if normalized:
    ...
    def draw_mask_cv(self, data, name, ref_name="img", color=(255, 0, 0)):
        ...
        mask = (
            (
                np.repeat(data[..., np.newaxis], 3, axis=2)
                * np.array(color).reshape(1, 1, -1)
            )
            .astype(np.uint8)
            .squeeze()
        )
    ...
    def draw_mask_cv_2(self, data, name, ref_name="img", color=(0, 0, 255)):
        ...
        for n, c in enumerate(color):
            image[:, :, n] = np.where(
                data == 1, image[:, :, n] * (1 - alpha) + alpha * c, image[:, :, n]
            )
