...
from libs.utils import to_np
from libs.hierarchy_encoding import hierarchy_encoding, grow_mask, comp_iou


def generate_shape(label, max_depth, process_mode):
    node_num = 360
    ...
    print('depth = %d, iou = %.2f' % (max_depth, iou))



image = cv2.imread("sample_data/image.png").transpose(2, 0, 1)
mask = cv2.imread("sample_data/image-mask.png").transpose(2, 0, 1)[0].astype(bool).astype(np.uint8)
h, w = mask.shape
...
label['id'] = 0
label['bbox'] = torch.tensor([[0,0,415,415]], dtype=torch.float)
...
for max_depth in range(6):
    generate_shape(label, max_depth, 'hybrid')
